...
set_temperature = html.Div(
    dbc.Card(
        [
            dbc.CardHeader(html.H4("Set Temperature", className="card-title")),
            dbc.CardBody(
                [
                    daq.Knob(
                        id="knob-temperature",
                        ...
                        scale={"start": 18, "labelInterval": 2, "interval": 1},
                    ),
...
app.layout = dbc.Container(
    [
        html.Div(
            [
                html.H1(
                    children="Daikin SkyFi AirCon Unit",
...
@app.callback(
    Output("knob-output-1", "children"), Input("knob-temperature", "value")
)
def update_output(value):
    return f"The temperature is set to {int(value)}."
...
def update_current_status(n):
    ...
    for z in ["Theatre", "Gym", "Family", "Beds"]:
        if z in aircon._zones:
            ...
            b = dbc.Button(
                ...
                children=z,
                color="success",
                style={"opacity": 1 if aircon._operation_mode else 0.3},
                className="flex-md-fill",
            )
            cards.append(b)
        else:
    ...
    aircon_temp = dbc.Card(
        [
            ...
            dbc.CardBody(
                html.P(
                    [
                        f"Aircon: {aircon_values['settemp']}",
                        html.Br(),
                        f"Room: {aircon_values['roomtemp']}",
                        html.Br(),
                        f"Outside: {aircon_values['outsidetemp']}",
                    ]
                ),
                className="card-text",
            ),
    ...
    cards = html.Div(
        cards, className="d-grid gap-2 d-md-flex justify-content-md-evenly"
    )
    return (
        " ".join([f"{i}:{j}" for i, j in aircon_values.items()]),
        f"Aircon is {on_off_options[int(aircon_values['opmode'])]}",
        cards,
        aircon_temp,
    )
...
def toggle_power(n):
    ...
    button = dbc.Button(
        "Turn Off" if aircon._operation_mode else "Turn On",
        color="primary" if aircon._operation_mode else "warning",
        className="flex-md-fill",
    )
    card_img = (
        "https://cdnl.iconscout.com/lottie/premium/thumb/air-conditioner-5558928-4647174.gif"
        if aircon._operation_mode
        else "https://miro.medium.com/v2/resize:fit:1400/1*Gvgic29bgoiGVLmI6AVbUg.gif"
    )
